...
-- Main definition of coinductive trees using a signature/container
mutual
  data STree {ℓ ℓ₁ ℓ₂} {K : Set ℓ} (I : K → Set ℓ₂) (A : Set ℓ₁) (i : Size) :
    Set (ℓ ⊔ ℓ₁ ⊔ ℓ₂) where
    leaf : (x : A) → STree I A i
    node : (k : K) (ts : I k → STree' I A i) → STree I A i

  record STree' {ℓ ℓ₁ ℓ₂} {K : Set ℓ} (I : K → Set ℓ₂) (A : Set ℓ₁) (i : Size) :
    Set (ℓ ⊔ ℓ₁ ⊔ ℓ₂) where
    coinductive
    field
...
-- Completed trees, given infinite size
...
-- Bisimilarity, a notion of equality for coinductive trees

mutual
  data Bisim {ℓ ℓ₁ ℓ₂} {K : Set ℓ₂} {I : K → Set ℓ₁} {A : Set ℓ} (i : Size) :
    Tree I A → Tree I A → Set (ℓ ⊔ ℓ₁ ⊔ ℓ₂) where
    leaf-eq : {x y : A} → (x ≡ y) → (Bisim i (leaf x) (leaf y))
    node-eq : ∀ {k ts ts'} → ((j : I k) → (Bisim' i (force (ts j)) (force (ts' j)))) →
      Bisim i (node k ts) (node k ts')

  record Bisim' {ℓ ℓ₁ ℓ₂} {K : Set ℓ₂} {I : K → Set ℓ₁} {A : Set ℓ} (i : Size)
    (t t' : Tree I A)  :  Set (ℓ ⊔ ℓ₁ ⊔ ℓ₂) where
    coinductive
    field
...
_∼_ :  ∀ {ℓ ℓ₁ ℓ₂} {K : Set ℓ₂} {I : K → Set ℓ₁} {A : Set ℓ} → Tree I A → Tree I A →
  Set (ℓ ⊔ ℓ₁ ⊔ ℓ₂)
t ∼ t' = Bisim ∞ t t'

-- The coinductive principle: Bisimilarity gives equality
postulate
  coin-prince :  ∀ {ℓ ℓ₁ ℓ₂} {K : Set ℓ₂} {I : K → Set ℓ₁} {A : Set ℓ} {t t' : Tree I A } →
    t ∼ t' → (t ≡ t')


-- Coinductive trees as a functor: action on maps
...
-- mapTree preserves function composition (needs function extensionality)
funcomTree :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃ ℓ₄} {K : Set ℓ₄} {I : K → Set ℓ₃} {A : Set ℓ} {B : Set ℓ₁}
  {C : Set ℓ₂} {i : Size} (f : A → B) (g : B → C) (t : Tree I A) →
  Bisim i (mapTree g (mapTree f t)) (mapTree (λ x → g (f x)) t)
funcomTree' :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃ ℓ₄} {K : Set ℓ₄} {I : K → Set ℓ₃} {A : Set ℓ} {B : Set ℓ₁}
  {C : Set ℓ₂} {i : Size} (f : A → B) (g : B → C) (t : Tree I A) →
  Bisim' i (mapTree g (mapTree f t)) (mapTree (λ x → g (f x)) t)
...
functTree :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃ ℓ₄} {K : Set ℓ₄} {I : K → Set ℓ₃} {A : Set ℓ} {B : Set ℓ₁}
  {C : Set ℓ₂} (f : A → B) (g : B → C) (t : Tree I A) →
  (mapTree g (mapTree f t)) ≡ (mapTree (λ x → g (f x)) t)
functTree f g t = coin-prince (funcomTree f g t)


-- Monad structure of the coinductive tree functor
...
η-is-nat :  ∀ {ℓ ℓ₁ ℓ₂} {A : Set ℓ} {B : Set ℓ₁} {K : Set ℓ₂} (I : K → Set ℓ₁) (f : A → B)
  (a : A) → (ηTree I (f a) ≡ mapTree f (ηTree I a))
η-is-nat f I a = refl

-- μ is a natural transformation
μ-is-nat :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃} {K : Set ℓ₃} {I : K → Set ℓ₂} {A : Set ℓ} {B : Set ℓ₁} {i : Size}
  (f : A → B) (t : Tree I (Tree I A))
...
μ-natural :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃} {K : Set ℓ₃} {I : K → Set ℓ₂} {A : Set ℓ} {B : Set ℓ₁}
  (f : A → B) (t : Tree I (Tree I A)) →
  (mapTree f (μTree t)) ≡ (μTree (mapTree (mapTree f) t))
μ-natural f t = coin-prince (μ-is-nat f t)

-- Monadic equations for eta
η-μ-law1 :  ∀ {ℓ ℓ₁ ℓ₂ ℓ₃} {K : Set ℓ₃} {I : K → Set ℓ₂} {A : Set ℓ} {B : Set ℓ₁} {i : Size}
  (t : (Tree I A)) →  t ≡ (μTree (ηTree I t))
η-μ-law1 t = refl

η-μ-law2-bis :  ∀ {ℓ ℓ₂ ℓ₃} {K : Set ℓ₃} {A : Set ℓ} {i : Size} {I : K → Set ℓ₂}
  (t : (Tree I A)) →  Bisim i t (μTree (mapTree (ηTree I) t))
η-μ-law2-bis' :  ∀ {ℓ ℓ₂ ℓ₃} {K : Set ℓ₃} {A : Set ℓ} {i : Size} {I : K → Set ℓ₂}
  (t : (Tree I A)) →  Bisim' i t (μTree (mapTree (ηTree I) t))
...
η-μ-law2 :  ∀ {ℓ ℓ₂ ℓ₃} {K : Set ℓ₃} {A : Set ℓ} {i : Size} {I : K → Set ℓ₂}
  (t : (Tree I A)) → t ≡ (μTree (mapTree (ηTree I) t))
...
-- The finality of the coalgebra of trees (coinductive trees as fixpoint)
